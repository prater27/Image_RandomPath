...
bool parserCommand(int argc, char ** argv, Mat &img , int &N, int &E,int &pointSize, int &lineSize, bool &drawRepeat, bool &drawPath, bool &onlyPath, int &cp) {
	const String keys =
		...
		"{closestPoints cp  |      | Random selection for the next point in the path among cp closestPoints}"
	...
	// Number of closest points when drawing the path
	cp = 5;
	if (parser.has("closestPoints"))
		cp = parser.get<int>("closestPoints");	
...
//Beggining of the code for the stippling
	...
	if (!parserCommand(argc, argv, img, N, E, pointSize, lineSize, drawRepeat, drawPath, onlyPath, cp )) {
		return -1;
	}
...
std::vector<pointWithDistance> pdVector;

for(int i=0; i<points.size();i++){
	pdVector.push_back(pointWithDistance(points[i].x, points[i].y, 0));
}

//Path finding happens here
pathFinder pathCalculation(pdVector, cp, points.size());
